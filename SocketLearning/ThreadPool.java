...
public class ThreadPool {				//线程池类
	...
	public static final int NORMAK_PRIORITY = 1;
	...
	protected synchronized void repool(PooledThread repoolingThread) {	//将线程重新放回池种的同步方法
		if(!shutDown_) {
			...
			case Thread.NORM_PRIORITY : {idleThreads_[NORMAK_PRIORITY].add(repoolingThread);}break;
	...
	public synchronized void start(Runnable target, int priority) { 
		...
		if(idleList.size() > 0) {			//存在空闲线程时直接使用空闲线程进行任务
			...
			case NORMAK_PRIORITY : thread.setPriority(Thread.NORM_PRIORITY);break;
	...
	public static void main(String[] args) {
		...
		pool.start(runner, NORMAK_PRIORITY);
